...
const res = await retryOnThrow(async () => (
  (await fetch(SOLEND_API_URL)).json() as Promise<ConfigType>
))
...
const parseTokenReserveData = (mint: PublicKey) => {
  const reserve = pool.reserves.find(({ liquidityToken }) => liquidityToken.mint === mint.toString())
  if (!reserve) {
  ...
  const collateralATAddress = getAssociatedTokenAddressSync(
